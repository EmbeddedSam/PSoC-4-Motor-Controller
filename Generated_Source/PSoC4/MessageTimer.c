...
void MessageTimer_Init(void)
{
    ...
    /* Set values from customizer to CTRL */
        ...
        MessageTimer_CONTROL_REG = MessageTimer_CTRL_QUAD_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        MessageTimer_TRIG_CONTROL1_REG  = MessageTimer_QUAD_SIGNALS_MODES;

        /* Set values from customizer to INTR */
        MessageTimer_SetInterruptMode(MessageTimer_QUAD_INTERRUPT_MASK);
        
         /* Set other values */
        MessageTimer_SetCounterMode(MessageTimer_COUNT_DOWN);
        MessageTimer_WritePeriod(MessageTimer_QUAD_PERIOD_INIT_VALUE);
        MessageTimer_WriteCounter(MessageTimer_QUAD_PERIOD_INIT_VALUE);
    #endif  /* (MessageTimer__QUAD == MessageTimer_CONFIG) */

    #if (MessageTimer__TIMER == MessageTimer_CONFIG)
        MessageTimer_CONTROL_REG = MessageTimer_CTRL_TIMER_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        MessageTimer_TRIG_CONTROL1_REG  = MessageTimer_TIMER_SIGNALS_MODES;
        ...
        #if (MessageTimer_CY_TCPWM_V2 && MessageTimer_TIMER_UPDOWN_CNT_USED && !MessageTimer_CY_TCPWM_4000)
            MessageTimer_WriteCounter(1u);
        #elif(MessageTimer__COUNT_DOWN == MessageTimer_TC_COUNTER_MODE)
            MessageTimer_WriteCounter(MessageTimer_TC_PERIOD_VALUE);
        #else
            MessageTimer_WriteCounter(0u);
        #endif /* (MessageTimer_CY_TCPWM_V2 && MessageTimer_TIMER_UPDOWN_CNT_USED && !MessageTimer_CY_TCPWM_4000) */
    #endif  /* (MessageTimer__TIMER == MessageTimer_CONFIG) */

    #if (MessageTimer__PWM_SEL == MessageTimer_CONFIG)
        MessageTimer_CONTROL_REG = MessageTimer_CTRL_PWM_BASE_CONFIG;

        #if (MessageTimer__PWM_PR == MessageTimer_PWM_MODE)
            MessageTimer_CONTROL_REG |= MessageTimer_CTRL_PWM_RUN_MODE;
            MessageTimer_WriteCounter(MessageTimer_PWM_PR_INIT_VALUE);
        #else
            MessageTimer_CONTROL_REG |= MessageTimer_CTRL_PWM_ALIGN | MessageTimer_CTRL_PWM_KILL_EVENT;
            
            /* Initialize counter value */
            #if (MessageTimer_CY_TCPWM_V2 && MessageTimer_PWM_UPDOWN_CNT_USED && !MessageTimer_CY_TCPWM_4000)
                MessageTimer_WriteCounter(1u);
            #elif (MessageTimer__RIGHT == MessageTimer_PWM_ALIGN)
                MessageTimer_WriteCounter(MessageTimer_PWM_PERIOD_VALUE);
            #else 
                MessageTimer_WriteCounter(0u);
            #endif  /* (MessageTimer_CY_TCPWM_V2 && MessageTimer_PWM_UPDOWN_CNT_USED && !MessageTimer_CY_TCPWM_4000) */
        #endif  /* (MessageTimer__PWM_PR == MessageTimer_PWM_MODE) */

        #if (MessageTimer__PWM_DT == MessageTimer_PWM_MODE)
            MessageTimer_CONTROL_REG |= MessageTimer_CTRL_PWM_DEAD_TIME_CYCLE;
        #endif  /* (MessageTimer__PWM_DT == MessageTimer_PWM_MODE) */

        #if (MessageTimer__PWM == MessageTimer_PWM_MODE)
            MessageTimer_CONTROL_REG |= MessageTimer_CTRL_PWM_PRESCALER;
        #endif  /* (MessageTimer__PWM == MessageTimer_PWM_MODE) */

        /* Set values from customizer to CTRL1 */
        MessageTimer_TRIG_CONTROL1_REG  = MessageTimer_PWM_SIGNALS_MODES;
...
*
* Summary:
*  Initializes the MessageTimer with default customizer
...
*  calls to the MessageTimer_Start() routine.
*
*******************************************************************************/
...
*  MessageTimer operates in the continuous mode.
*
* Parameters:
*  oneShotEnable
*   Values:
...
void MessageTimer_WriteCompare(uint32 compare)
{
    ...
    #if (MessageTimer_CY_TCPWM_4000)
        currentMode = ((MessageTimer_CONTROL_REG & MessageTimer_UPDOWN_MASK) >> MessageTimer_UPDOWN_SHIFT);

        if (((uint32)MessageTimer__COUNT_DOWN == currentMode) && (0xFFFFu != compare))
        {
            compare++;
        }
        else if (((uint32)MessageTimer__COUNT_UP == currentMode) && (0u != compare))
        {
            compare--;
        }
        else
...
uint32 MessageTimer_ReadCompare(void)
{
        ...
        regVal = MessageTimer_COMP_CAP_REG;
        
        if (((uint32)MessageTimer__COUNT_DOWN == currentMode) && (0u != regVal))
        {
            regVal--;
        }
        else if (((uint32)MessageTimer__COUNT_UP == currentMode) && (0xFFFFu != regVal))
        {
            regVal++;
        }
        else
        ...
        return (regVal & MessageTimer_16BIT_MASK);
    #else
...
void MessageTimer_WriteCompareBuf(uint32 compareBuf)
{
        ...
        if (((uint32)MessageTimer__COUNT_DOWN == currentMode) && (0xFFFFu != compareBuf))
        {
            compareBuf++;
        }
        else if (((uint32)MessageTimer__COUNT_UP == currentMode) && (0u != compareBuf))
        {
            compareBuf --;
        }
        else
...
uint32 MessageTimer_ReadCompareBuf(void)
{
        ...
        regVal = MessageTimer_COMP_CAP_BUF_REG;
